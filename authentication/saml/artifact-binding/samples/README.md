---
title: "HTTP-Artifact Binding — Sample Capture"
creation: 2026-07-25
...
# HTTP-Artifact Binding — Sample Capture
...
- HAR capture: [`artifact-binding.har`](./artifact-binding.har) — HTTP-level view of the exchange (load it into the browser Network tab or a HAR viewer).
...
With the artifact binding this `Response` is **not** in the browser POST; the SP obtains it by resolving `SAMLart` over the back channel. The decoded `ArtifactResolve` and the resolved `Response` are shown.

### Back-channel `ArtifactResolve` (SP → IdP ARS, SOAP)
...
- **`Artifact`** — the opaque 44-byte reference (base64) the browser carried in `SAMLart`; one-time-use, short TTL.

### Resolved `Response` (inside the `ArtifactResponse`)
