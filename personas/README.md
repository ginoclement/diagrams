...
| **Break-glass** | Sealed local credential, bypasses federation | Offline/second-channel factor; alert on use | Provisioned once, sealed; periodic rehearsal | Created deliberately; reviewed, never auto-JML | **Reseal + rotate** after any use; no SSPR (by-persona *(planned)*) | Every use audited; existence reviewed (by-persona *(planned)*) | Standing max privilege, use-gated + alerted |
| **Developer** | [OIDC dev client](../oidc/authorization-code-pkce/README.md) + [device-code](../oidc/device-authorization/README.md) for CLIs | MFA on human auth; tokens short-lived | Self-service key/PAT/client issuance ([by-persona](enrollment-by-persona/README.md)) | Workforce lifecycle for the human; keys rotate/expire | Reissue PAT/key; short-lived token refresh (by-persona *(planned)*) | Key/PAT usage + scope review | Scopes/claims *(planned)*; least-privilege tokens |
...
- credential-recovery-by-persona *(planned)* — consumer SSPR
  vs workforce SSPR+helpdesk vs privileged vaulted (no direct reset) vs workload rotate.
- access-review-by-persona *(planned)* — cadence and approver
...
- [RBAC](../authorization/rbac/README.md) / [ABAC](../authorization/abac/README.md) /
  Scopes & Claims *(planned)* — authorization models referenced above.
...
- credential-recovery-by-persona/ *(planned)*
- access-review-by-persona/ *(planned)*
